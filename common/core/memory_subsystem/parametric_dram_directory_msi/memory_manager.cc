   ...
   m_enabled(false)
{
   ...
   std::map<MemComponent::component_t, CacheParameters> cache_parameters;// hash table for saving cache parameters
   std::map<MemComponent::component_t, String> cache_names;		// Hash Table for saving cache names
...
//-----------------------Generating String name for caches
...
// Saving Cache Parameters in hash table cache_parameters for ith memory........................................................
...
//-----------------------------------------------------------------------------------------------------------------------
		//---save cache names in hash table cache_names	
...
///------------------------------------------------------------------------------------------
   ...
   for(UInt32 i = MemComponent::FIRST_LEVEL_CACHE; i <= (UInt32)m_last_level_cache; ++i) 
   {
	...
	//------INitializing cache controllerrs
	 CacheCntlr* cache_cntlr = new CacheCntlr(
         ...
         getCacheBlockSize(),
         cache_parameters[(MemComponent::component_t)i],getShmemPerfModel(),i == (UInt32)m_last_level_cache
      );
	
	PRAK_LOG("core %d Allocating cache_cntlr in paramteric mem_man comp=%s ",getCore()->getId(), MemComponentString		((MemComponent::component_t)i));
...
//-------------------------Cache performance and latency goes here
   ...
   if (m_core_id_master == getCore()->getId())
   {
      ...
      if (dram_direct_access && getCore()->getId() < (core_id_t)Sim()->getConfig()->getApplicationCores())
      {
		...
		PRAK_LOG("SETTING DIRECT DRAM ACCESS MEM_MANAGER");	         
