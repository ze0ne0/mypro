...
//--------------PRAK
	...
	fptr=fopen(Sim()->getConfig()->formatOutputFileName(filename).c_str(), "w");
...
//------------PRAK------------
	...
	{
		m_memory_manager->getCacheCntlrAt(m_core_id, MemComponent::L2_CACHE)->getSlabCntlr()->setTime();
	}
//	VERI_LOG("INC");
/*
	if(m_core_id==0)
...
//------------------------------
   ...
   if ((address & blockmask) == m_icache_last_block)
   {
      ...
      {
	//	PRAK_LOG("Instr Addr=0x%x found in L1I cache last buff and in single line",address );
	...
	//PRAK_LOG("Instr Addr=0x%x found in L1I cache last buff and in multiple line",address );
      }
   }
   ...
   m_icache_last_block = address & blockmask;
	...
	//PRAK_LOG("Addr=0x%x initiating access to Memcomponent=%s ",address,MemComponentString(MemComponent::L1_ICACHE));
